...
function StudentList() {
  ...
  const handleSearchByName = (input) => {
    const results = students.filter(({ firstName = "", lastName = "" }) =>
  ...
  const handleSearchByTag = (input) => {
    ...
    const filteredTags = tags.filter(({ tag = "" }) =>
      [tag, `${tag}`].some((el) => el.includes(input))
    );
    if (input !== "") {
  ...
  useEffect(
    () => {
      ...
      const result = nameResult.filter((obj1) =>
        tagResult.some((obj2) => obj1.id === obj2.id)
      );
